...
export function InteractivePricing({ tiers }: { tiers: PricingTier[] }) {
  ...
  return (
    <section id="pricing" className="w-full bg-gray-100 py-16">
      <div className="container mx-auto text-center">
        ...
        <div className="mt-12 flex flex-col justify-center gap-8 md:flex-row">
          {tiers.map((tier, index) => (
            <motion.div
              key={index}
              className={`flex-1 cursor-pointer rounded-xl bg-white p-8 shadow-lg transition ${
            ...
            >
              ...
              <p className="mt-2 text-lg text-gray-600">{tier.speed}</p>
              ...
              <button
                className={`mt-8 w-full rounded-full px-6 py-3 text-lg font-semibold transition ${
                  tier.highlight ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-200 hover:bg-gray-300'
                }`}
              >
                Get Started
              </button>
